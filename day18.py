...
def solve():
    ...
    for i, empty_c in enumerate(empty_cubes):
        if i % 1000 == 0:
            print(f"{i/len(empty_cubes):%} completed", end="\r")
        if empty_c in known_outside or empty_c in known_inside:
            pass
        is_out, searched = connected_to_outside(
            empty_c, all_cubes, bounding_box
        )
        if is_out:
    ...
    for cube in all_cubes:
        for i in range(3):
            for sign in [+1, -1]:
                coords = list(cube)
                coords[i] += sign
                n_exposed_faces += tuple(coords) not in all_cubes
                empty_cubes.append(tuple(coords))

    return n_exposed_faces, empty_cubes


def connected_to_outside(empty_space, all_cubes, bounding_box):
